...
const SourceSection = ({ data }: SourceSectionProps) => {
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-panel p-4">
          ...
          <p className="text-xl font-bold">{typeof data?.realtime.ratio === 'number' ? data.realtime.ratio.toFixed(2) : '--'}</p>
        </div>
        <div className="glass-panel p-4">
          <p className="text-xs text-muted-foreground">Trend (µg/m³/h)</p>
          <p className="text-xl font-bold">{typeof data?.realtime.trend === 'number' ? data.realtime.trend.toFixed(1) : '--'}</p>
        </div>
        <div className="glass-panel p-4">
          <p className="text-xs text-muted-foreground">Volatilità</p>
          <p className="text-xl font-bold">{typeof data?.realtime.volatility === 'number' ? data.realtime.volatility.toFixed(2) : '--'}</p>
        </div>
        <div className="glass-panel p-4">
          <p className="text-xs text-muted-foreground">PM2.5 attuale</p>
          <p className="text-xl font-bold">{typeof data?.realtime.pm25 === 'number' ? data.realtime.pm25.toFixed(1) : '--'}</p>
        </div>
      </div>
    </motion.div>
  );
